...
axes[0].set_title("Lax-Friedrich Advection", fontsize=15)
axes[1].set_title("Implicit Diffusion", fontsize=15)
...
while i < nsteps:
    ...
    # implicit diffusion
    A = np.eye(n) * (1 + 2 * beta) + (-beta) * np.eye(n, k=1) + (-beta) * np.eye(n, k=-1)
